...
// src/transports.ts
async function dispatchPopup(t, ctx) {
  const w = ctx.windowRef ?? globalThis.window;
  if (!w) return { kind: "unavailable", reason: "no window object (server context)" };
  const url = new URL(t.walletUrl);
  url.searchParams.set("req", encodeRpVerifyRequest(ctx.request));
  const popup = w.open(
    url.toString(),
    "numkeys-verify",
    t.popupFeatures ?? "popup,width=480,height=720"
  );
  if (!popup) return { kind: "unavailable", reason: "popup blocked by browser" };
  const walletOrigin = new URL(t.walletUrl).origin;
  return new Promise((resolve) => {
    ...
    const onMessage = (ev) => {
      ...
      if (errParse.success) {
        if (errParse.data.request_id !== ctx.request.request_id) return;
        settle({ kind: "got_response", response: errParse.data });
      } else if (okParse.success && okParse.data.request_id === ctx.request.request_id) {
        settle({ kind: "got_response", response: okParse.data });
      }
    };
    w.addEventListener("message", onMessage);
    const closedPoll = setInterval(() => {
      if (popup.closed) settle({ kind: "unavailable", reason: "user closed popup" });
    }, 500);
...
async function dispatchExtension(t, ctx) {
  const w = ctx.windowRef ?? globalThis.window;
  if (!w) return { kind: "unavailable", reason: "no window object" };
  const txId = randomTxId();
  const probed = await new Promise((resolve) => {
    let acked = false;
    const onProbeReply = (ev) => {
      if (ev.source !== w) return;
      const d = ev.data;
      if (d?.type === "numkeys/sign-ack" && d.tx_id === txId) {
        acked = true;
        w.removeEventListener("message", onProbeReply);
        resolve(true);
      }
    };
    w.addEventListener("message", onProbeReply);
    w.postMessage({ type: "numkeys/sign-probe", tx_id: txId }, ctx.rpOrigin);
    setTimeout(() => {
      if (!acked) {
        w.removeEventListener("message", onProbeReply);
        resolve(false);
      }
    }, t.probeTimeoutMs ?? 200);
  });
  if (!probed) {
    return { kind: "unavailable", reason: "no extension responded to probe" };
  }
  return new Promise((resolve) => {
    let settled = false;
    const settle = (o) => {
      if (settled) return;
      settled = true;
      w.removeEventListener("message", onResponse);
      clearTimeout(timer);
      resolve(o);
    };
    const onResponse = (ev) => {
      if (ev.source !== w) return;
      const d = ev.data;
      if (d?.type !== "numkeys/sign-response") return;
      if (d.tx_id !== txId) return;
      if (d.request_id !== ctx.request.request_id) return;
      const okParse = RpVerifyResponseSchema.safeParse(d.response);
      const errParse = RpVerifyErrorResponseSchema.safeParse(d.response);
      if (errParse.success) settle({ kind: "got_response", response: errParse.data });
      else if (okParse.success) settle({ kind: "got_response", response: okParse.data });
    };
    w.addEventListener("message", onResponse);
    w.postMessage(
      { type: "numkeys/sign-request", tx_id: txId, request: ctx.request },
      ctx.rpOrigin
    );
    const timer = setTimeout(
      () => settle({ kind: "timeout" }),
      t.responseTimeoutMs ?? 5 * 60 * 1e3
    );
  });
}
async function dispatchRedirect(t, ctx) {
  const w = ctx.windowRef ?? globalThis.window;
  if (!w) return { kind: "unavailable", reason: "no window object" };
  if (!ctx.request.return_url || new URL(ctx.request.return_url).href !== new URL(t.returnUrl).href) {
    return {
      kind: "unavailable",
      reason: "request.return_url does not match transport.returnUrl"
    };
  }
  if (t.persistRequest) {
    try {
      t.persistRequest(ctx.request);
    } catch (e) {
      return { kind: "unavailable", reason: `persistRequest threw: ${e.message}` };
    }
  }
  const url = new URL(t.walletUrl);
  url.searchParams.set("req", encodeRpVerifyRequest(ctx.request));
...
async function dispatchQr(t, ctx) {
  let session;
  try {
    session = await t.createSession();
  } catch (e) {
    return { kind: "unavailable", reason: `createSession failed: ${e.message}` };
  }
  const encodedReq = encodeRpVerifyRequest(ctx.request);
  const qrPayload = `numkeys://sign?session=${encodeURIComponent(session.signRequestUrl)}&req=${encodedReq}`;
  let dismissed = false;
  const dismiss = () => {
    if (dismissed) return;
    dismissed = true;
  ...
  try {
    await t.displayQr(qrPayload);
  } catch (e) {
    dismiss();
    return { kind: "unavailable", reason: `displayQr failed: ${e.message}` };
  }
  const interval = t.pollIntervalMs ?? 1500;
  const timeout = t.timeoutMs ?? 5 * 60 * 1e3;
  const deadline = Date.now() + timeout;
  while (Date.now() < deadline) {
    try {
      const r = await fetch(session.pollUrl, { method: "GET", cache: "no-store" });
      if (r.ok) {
        ...
        if (text && text !== "null") {
          ...
          try {
            ...
            try {
              raw = JSON.parse(new TextDecoder().decode(b64UrlToBytes(text)));
            } catch {
              raw = null;
            }
          }
          ...
          if (errParse.success && errParse.data.request_id === ctx.request.request_id) {
            dismiss();
            return { kind: "got_response", response: errParse.data };
          }
          if (okParse.success && okParse.data.request_id === ctx.request.request_id) {
            dismiss();
            return { kind: "got_response", response: okParse.data };
          }
        }
      }
    } catch {
...
// src/request-attestation.ts
async function requestAttestation(params) {
  if (params.transports.length === 0) {
    return { ok: false, reason: "no_transport_available", tried: [] };
  }
  const redirect = params.transports.find(
    (t) => t.kind === "redirect"
  );
  if (redirect) {
    let returnOrigin;
    let rpOrigin;
    try {
      returnOrigin = new URL(redirect.returnUrl).origin;
      rpOrigin = new URL(params.rpOrigin).origin;
    } catch (e) {
      return {
        ok: false,
        reason: "no_transport_available",
        tried: [
          { transport: "redirect", reason: `invalid url: ${e.message}` }
        ]
      };
    }
    if (returnOrigin !== rpOrigin) {
      return {
        ok: false,
        reason: "no_transport_available",
        tried: [
          {
            transport: "redirect",
            reason: `returnUrl origin (${returnOrigin}) does not match rpOrigin (${rpOrigin})`
          }
        ]
      };
    }
  ...
  try {
    ...
    return {
      ok: false,
      reason: "no_transport_available",
      tried: [{ transport: params.transports[0].kind, reason: `invalid request params: ${e.message}` }]
    };
  }
  const tried = [];
  const ctx = { request, rpOrigin: params.rpOrigin, windowRef: params.windowRef };
  for (const transport of params.transports) {
    let outcome;
    try {
      switch (transport.kind) {
        case "popup":
          outcome = await dispatchPopup(transport, ctx);
          break;
        case "extension":
          outcome = await dispatchExtension(transport, ctx);
          break;
        case "redirect":
          outcome = await dispatchRedirect(transport, ctx);
          break;
        case "qr":
          outcome = await dispatchQr(transport, ctx);
          break;
      }
    } catch (e) {
      outcome = { kind: "unavailable", reason: `transport threw: ${e.message}` };
    }
    switch (outcome.kind) {
      case "unavailable":
        tried.push({ transport: transport.kind, reason: outcome.reason });
        continue;
      case "timeout":
        return { ok: false, reason: "timeout", transport: transport.kind };
      case "navigated":
        return { ok: false, reason: "navigated", transport: "redirect", request };
      case "got_response": {
        ...
        return verified.ok ? { ok: true, transport: transport.kind, result: verified.result, response: verified.response } : verified.kind === "declined" ? { ok: false, reason: "declined", transport: transport.kind, error: verified.error } : { ok: false, reason: "verification_failed", transport: transport.kind, error: verified.error };
      }
    }
  }
  return { ok: false, reason: "no_transport_available", tried };
}
async function verifyResponse(request, response, params) {
  if ("error" in response) {
    return { ok: false, kind: "declined", error: response.error };
  }
  ...
  try {
    const result = await verifyRpVerifyResponse({
      ...
      response,
      options: {
        allowedIssuers: params.trustPolicy.allowedIssuers,
        maxAttestationAgeSec: params.trustPolicy.maxAttestationAgeSec ?? 60 * 60 * 24 * 365,
        clockSkewSec: params.trustPolicy.clockSkewSec ?? 60
      },
    ...
    return { ok: false, kind: "verification_failed", error: msg };
  }
}
...
export { InMemoryNonceStore, NUMKEYS_PROTOCOL_VERSION, ProtocolError, WALLET_SDK_CONTRACT_VERSION, b64UrlToBytes, buildRpVerifyRequest, bytesToB64Url, canonicalJson, consumeRedirectFragment, encodeRpVerifyRequest, openVerifyPopup, requestAttestation, verifyRpVerifyResponse };
